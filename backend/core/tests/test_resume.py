...
def test_parse_resume_text_sections():
    ...
    assert draft["headline"] == "Anna Svensson"
    assert "fem års erfarenhet" in draft["summary"]
    assert draft["skills"] == ["Python", "Django", "PostgreSQL", "Docker"]
    assert [row["title"] for row in draft["experience"]] == [
        "Backendutvecklare, Acme AB 2021-2024",
        "Systemutvecklare, Initech 2019-2021",
    ]
    assert draft["education"][0]["school"] == "Systemvetenskap, Lunds universitet"
